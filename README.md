...
* **Super Portátil:** Ele foi desenhado sob medida para as rotas do **Steam Deck (SteamOS)**, mas conta com um detector inteligente de dependências e gerenciadores de pacotes. Isso significa que ele vai rodar liso e tentar instalar o `chntpw` sozinho se você levá-lo para o **Ubuntu, Fedora, Arch, openSUSE**, etc.

---

### ⚠️ A Regra de Ouro (LEIA ANTES DE RODAR!)
...
2. Baixe o script `sync-bluetooth.sh` e o atalho `Sincronizar_Bluetooth.desktop` para a sua Área de Trabalho.
...
4. Abra o arquivo `.desktop` com um editor de texto e ajuste o caminho da linha `Exec=` para onde o seu script `.sh` está salvo.
5. Dê dois cliques no atalho da Área de Trabalho. Ele vai abrir o terminal Konsole sozinho, pedir a sua senha de `sudo` e fazer toda a mágica na sua frente.
